...
class AICodeReviewer {
  ...
  private analyzeCodeQuality(filePath: string, diff: string): string[] {
    ...
    if (diff.match(/useEffect\(\s*\(\)\s*=>\s*{[^}]*}\s*\)/)) {
      issues.push('useEffect with missing dependency array - will run on every render');
    }
  ...
  private generateSuggestions(filePath: string, diff: string, issues: string[]): string[] {
    ...
    if (issues.includes('useEffect with missing dependency array - will run on every render')) {
      suggestions.push('Add dependency array to useEffect to control when it runs ([] for once, [deps] for when deps change)');
    }
