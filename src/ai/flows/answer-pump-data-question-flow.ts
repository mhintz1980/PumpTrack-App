...
const GetShippedPumpsInputSchema = z.object({
  ...
  month: z.string().describe("The full name of the month, e.g., 'April', 'May'. Case-insensitive."),
...
const GetShippedPumpsOutputSchema = z.object({
  ...
  message: z.string().optional().describe("Additional context, e.g. 'No DD4SE pumps were shipped in April 2024.'"),
});
...
const GetPumpsToVendorInputSchema = z.object({
  model: z.string().optional().describe("The pump model. If not specified, query for all models."),
  vendorName: z.string().describe("The name of the powder coat vendor."),
  timePeriod: z.string().describe("The time period for pumps sent, e.g., 'last 3 weeks', 'this month'. Can be relative."),
});
const GetPumpsToVendorOutputSchema = z.object({
  model: z.string().optional(),
  vendorName: z.string(),
  timePeriod: z.string(),
  countSent: z.number().describe("Number of pumps sent to the vendor in the time period."),
  currentCountAtVendor: z.number().describe("Number of pumps currently at the vendor."),
...
const GetCustomerOrderStatusOutputSchema = z.object({
  ...
  openOrdersSummary: z.array(z.object({
    poNumber: z.string(),
    model: z.string(),
    serialNumber: z.string().optional(),
    currentStage: z.string(),
    quantity: z.number().optional().describe("Quantity for this PO if it's a batch order without individual serials yet."),
  })).optional().describe("Summary of open orders. Can be empty if no open orders."),
...
const GetLastShipmentForPOOutputSchema = z.object({
  poNumber: z.string(),
  lastShipmentDate: z.string().optional().describe("Date of the last shipment (YYYY-MM-DD)."),
  status: z.enum(["SHIPPED", "PARTIALLY_SHIPPED", "NOT_SHIPPED_YET", "PO_NOT_FOUND", "PO_NOT_FULLY_SHIPPED"]),
  message: z.string().optional(),
});
...
const getShippedPumpsTool = ai.defineTool(
  {
    ...
    description: 'Finds out how many pumps of a specific model were shipped in a given month and year.',
  ...
  async (input) => {
    ...
    if (input.model.toUpperCase() === 'DD4SE' && input.month.toLowerCase() === 'april' && input.year === 2024) {
      return { model: input.model, month: input.month, year: input.year, count: 12, status: "FOUND", message: "12 DD4SE pumps were shipped in April 2024." };
    }
    return { model: input.model, month: input.month, year: input.year, count: 0, status: "NO_DATA", message: `No data for ${input.model} shipped in ${input.month} ${input.year}.` };
  }
);
...
const getPumpsToVendorTool = ai.defineTool(
  {
    ...
    description: 'Checks how many pumps (optionally of a specific model) were sent to a powder coat vendor in a given time period, and how many are currently there.',
    inputSchema: GetPumpsToVendorInputSchema,
    outputSchema: GetPumpsToVendorOutputSchema,
  },
  async (input) => {
    // !!! Placeholder: Actual database query would go here !!!
    console.log(`[Tool:getPumpsToVendorTool] Called with: ${JSON.stringify(input)}`);
    if (input.vendorName.toLowerCase().includes('acme') && input.model?.toUpperCase() === 'RL200') {
      return {
        model: input.model,
    ...
    return { model: input.model, vendorName: input.vendorName, timePeriod: input.timePeriod, countSent: 0, currentCountAtVendor: 0, status: "NO_DATA" };
  }
);
...
const getCustomerOrderStatusTool = ai.defineTool(
  {
    ...
    description: 'Provides a summary of all open (not fully shipped) orders for a specific customer.',
    inputSchema: GetCustomerOrderStatusInputSchema,
    outputSchema: GetCustomerOrderStatusOutputSchema,
  },
  async (input) => {
    // !!! Placeholder: Actual database query would go here !!!
    console.log(`[Tool:getCustomerOrderStatusTool] Called with: ${JSON.stringify(input)}`);
    if (input.customerName.toLowerCase().includes('sunbelt')) {
      return {
        customerName: input.customerName,
        openOrdersSummary: [
          { poNumber: "PO12345", model: "DD6", serialNumber: "MSP-JN-1021", currentStage: "Assembly" },
          { poNumber: "PO67890", model: "RL200", quantity: 5, currentStage: "Fabrication" },
        ],
        status: "FOUND",
        message: "Sunbelt Rentals has 2 open orders: PO12345 (DD6, S/N MSP-JN-1021) is in Assembly. PO67890 (5x RL200) is in Fabrication."
      };
    }
    return { customerName: input.customerName, status: "NO_OPEN_ORDERS", message: `No open orders found for ${input.customerName}.` };
  }
);
...
const getLastShipmentForPOTool = ai.defineTool(
  {
    ...
    description: 'Finds the shipment date of the last pump for a given Purchase Order (PO) number.',
    inputSchema: GetLastShipmentForPOInputSchema,
    outputSchema: GetLastShipmentForPOOutputSchema,
  },
  async (input) => {
    // !!! Placeholder: Actual database query would go here !!!
    console.log(`[Tool:getLastShipmentForPOTool] Called with: ${JSON.stringify(input)}`);
    if (input.poNumber === 'PO3456') {
      return { poNumber: input.poNumber, lastShipmentDate: "2024-05-15", status: "SHIPPED", message: "The last pump on PO3456 shipped on 2024-05-15." };
    }
    return { poNumber: input.poNumber, status: "PO_NOT_FOUND", message: `PO ${input.poNumber} not found or not shipped.` };
  }
);
...
const AIResponseOutputSchema = z.object({
  answer: z.string().describe("The AI-generated answer to the user's question, based on tool outputs if used."),
});
  ...
  async (input) => {
    ...
    if (history && history.length > 0) {
      history.forEach((event, index) => {
        console.log(`[AI History Event ${index}]: Role: ${event.role}, Parts:`, event.parts);
        event.parts.forEach(part => {
          if (part.toolRequest) console.log(`  Tool Request: ${part.toolRequest.name}, Input: ${JSON.stringify(part.toolRequest.input)}`);
          if (part.toolResponse) console.log(`  Tool Response: ${part.toolResponse.name}, Output: ${JSON.stringify(part.toolResponse.output)}`);
        });
      });
    }
