...
export function KanbanBoard({
  ...
  // Props reserved for future view mode feature
  void columnViewModes;
  void onToggleColumnViewMode;
  const [draggedItemInfo, setDraggedItemInfo] = React.useState<
    { type: "single"; id: string } | { type: "group"; pumpIds: string[] } | null
  >(null);

  const handleDragStartSingle = (
    e: React.DragEvent<HTMLDivElement>,
    pumpId: string,
  ) => {
    if (
      selectedPumpIdsForDrag.includes(pumpId) &&
      selectedPumpIdsForDrag.length > 1
    ) {
      setDraggedItemInfo({ type: "group", pumpIds: selectedPumpIdsForDrag });
      e.dataTransfer.setData(
        "application/pump-group-ids",
        JSON.stringify(selectedPumpIdsForDrag),
      );
    } else {
  ...
  const handleDrop = (
    ...
    if (draggedItemInfo.type === "single") {
      const pumpIdFromDataTransfer = e.dataTransfer.getData(
        "application/pump-id",
      );
      ...
      const pumpGroupIdsFromDataTransfer = e.dataTransfer.getData(
        "application/pump-group-ids",
      );
      try {
        const parsedPumpIds = JSON.parse(
          pumpGroupIdsFromDataTransfer,
        ) as string[];
        if (
          Array.isArray(parsedPumpIds) &&
          parsedPumpIds.every((id) => typeof id === "string") &&
          JSON.stringify(parsedPumpIds.sort()) ===
            JSON.stringify(draggedItemInfo.pumpIds.sort())
        ) {
